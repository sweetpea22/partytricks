...
import { networkConfig, developmentChains, gatewayGoerli, gasServiceGoerli, gatewayBase, gasServiceBase, gasServiceFuji, gatewayFuji } from "../deploy-config"
...
const deploySendVoteCrossChain: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const sendVoteCrossChainBase = await deploy("SendVoteCrossChain", {
    from: deployer,
    args: argsBase,
    log: true,
    // we need to wait if on a live network so we can verify properly
